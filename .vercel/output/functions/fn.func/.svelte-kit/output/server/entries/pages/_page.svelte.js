...
const TabGroup = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	<div class="${'tab-list ' + escape(classesList, true)}" role="tablist"${add_attribute(
		'aria-labelledby',
		labelledby,
		0
...
const Tab = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	<div class="${'tab ' + escape(classesTab, true)}" data-testid="tab" role="tab"${add_attribute(
		...
		<div class="${'tab-interface ' + escape(classesInterface, true)}">${
...
const Athletes = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<div class="flex grid grid-cols-3 md:grid-cols-4 gap-4">${each(listData, (v) => {
		return `<div class="col-span-1"><a href="${'/' + escape(v.link, true)}"><img${add_attribute(
			'src',
			v.avatar,
			0
		)} alt="profile"></a>
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	do {
		$$settled = true;
		$$rendered = `${
			(($$result.head += `<!-- HEAD_svelte-19mtgnf_START -->${
				(($$result.title = `<title>${escape(title)}</title>`), '')
			}<!-- HEAD_svelte-19mtgnf_END -->`),
			'')
		}
	...
	<div><div class="mx-auto max-w-7xl text-token grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4">
			<div class="col-span-3 card card-hover overflow-hidden"><header><img src="/images/banner2.png" class="bg-black/50 w-full " alt="Post"></header>
				<div class="p-4 space-y-4">${validate_component(TabGroup, 'TabGroup').$$render(
					$$result,
					{},
					{},
					{
						panel: () => {
							...
							${
								...
								<section>${each(data.posts, (post) => {
									return `<div class="p-4"><span class="flex justify-between"><h3 class="h3" data-toc-ignore><a${add_attribute(
										'href',
										post.slug,
										0
									)} class="title">${escape(
										post.title
									)}</a></h3><div class="justify-around">${validate_component(
										Avatar,
										'Avatar'
									).$$render($$result, { src: post.avatar, width: 'w-10' }, {}, {})}</div></span>							
									...
									: `${
											tabsBasic === 1
												? `<p>${escape(desc.popular)}</p>`
												: `${tabsBasic === 2 ? `<p>${escape(desc.following)}</p>` : ``}`
									  }`
							}
						`;
						},
			...
			<div class="col-span-2 card card-hover overflow-hidden"><h3 class="p-4 h3">Newsletter</h3>
				...
				<div class="flex justify-center">${validate_component(Subscribe, 'Subscribe').$$render(
					$$result,
					{},
					{},
					{}
